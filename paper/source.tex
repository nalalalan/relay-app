...
\vspace{0.35em}
\noindent\textbf{Autonomous agents are increasingly able to use tools, write messages, and execute workflows, but deployed claims of autonomy are difficult to evaluate when actions, constraints, market signals, and commercial outcomes are scattered across product analytics and human memory. We report Relay, a continuously updated deployment record for a notes-first revenue agent governed by an explicit proof contract. The system converts a broad commercial goal into a state machine with recorded bottlenecks, permitted actions, forbidden actions, proof deadlines, buyer-intent signals, payment outcomes, and compact research-journal entries. At the initial evidence lock, Relay exposed nine production routes, had all eleven checked environment requirements present, retained 30-day telemetry including 56 page views, 8 link clicks, 3 checkout clicks, and 3 lead-capture events, and recorded four compact research-journal entries. In the latest live check, read from production at approximately 2026-05-14T20:17Z, the seven-day public-intent window contained 43 page views and 6 sample clicks, while the current active outbound proof \texttt{hard\_paid\_test\_direct} had completed 10/10 active-sample sends with 0 replies, 0 payments, USD~0.00 gross revenue, and a reply-observation deadline at 2026-05-15T13:30:12Z. The dashboard now exposes first-money implementation readiness as 11/12 gates complete: the USD~1 checkout, product route, checkout-first page, single checkout path, notes fallback, telemetry, active sample, sample inspection, paid-fulfillment check, reply-window protection, and next one-variable rotation are present, while the real paid-buyer gate remains missing. Source inspection also identified a state-consistency fault: one summary contract could request rotation while the proof controller was still inside the reply-observation window. The current source resolves that contradiction by returning \texttt{observe\_reply\_window} until the deadline matures. The contribution is therefore not a claim that the system already makes money. It is a falsifiable protocol for making autonomous commercial experimentation auditable enough that future success, failure, and execution regressions can be interpreted.}
...
The latest live check was read from the production Relay API at approximately 2026-05-14T20:17Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The current active experiment is \texttt{hard\_paid\_test\_direct}: ten active first-touch sends were observed, the active sample was complete, and no buyer replies or payments had arrived. The most important current observation is therefore operational rather than commercial. The send sample is complete, the paid outcome is still zero, and the live controller is waiting until 2026-05-15T13:30:12Z before changing copy or targeting. The source now makes that wait state explicit across the money-decision contract, success governor, owner-absence contract, and autonomous-money mandate, so a completed sample cannot simultaneously be displayed as both ``rotate now'' and ``wait for observation''.
...
\midrule
One-day intent & 4 page views, 8 page-engagement events, 2 sample clicks, 0 leads. \\
Seven-day intent & 43 page views, 76 page-engagement events, 6 sample clicks, 0 note-intake clicks, 0 checkout clicks, 0 leads. \\
Revenue & USD~0.00 gross revenue; 0 payments. \\
Rolling outbound & 50 sends over seven days; 0 replies; 0 payments; 0 send failures. \\
Active experiment & \texttt{hard\_paid\_test\_direct}; 10/10 active-sample sends observed; reply observation pending after the last active send at 2026-05-14T13:30:12Z. \\
Controller state & The controller state is now normalised to \texttt{observe\_reply\_window} until 2026-05-15T13:30:12Z; rotation is blocked while the reply-observation window is active. \\
First-money gates & 11/12 implementation gates complete: USD~1 checkout, product route, checkout-first page, single checkout path, notes fallback, telemetry, 10/10 sample, sample opens, paid-fulfillment check, reply-window protection, and the next one-variable rotation are present; the real paid-buyer gate remains missing. \\
...
The send-day accounting was revised so daily send counts begin at midnight in the configured send timezone rather than midnight UTC. The active paid-test path was also revised to expose a direct USD~1 first-money checkout, make the checkout-first route the first-screen action, add a source-visible primary-money-path marker, and preserve a working \texttt{\#send-notes} fallback for prospects who want to send rough notes before paying. The latest source revision additionally makes the reply-observation state authoritative across all money-control summaries and replaces the bottom dashboard checklist with a compact first-money sequence: pay path, traffic, inspection, sample, next angle, first payment. This update is recorded because an autonomous money-making system must learn from readiness, buyer-friction, state-consistency, and execution-accounting failures, not only from buyer outcomes.
...
No hypothesis test was performed. The initial evidence-lock observations were descriptive: 56 page views, 18 filtered real page views, 8 link clicks, 3 checkout clicks, 3 lead-capture events, 1 filtered real lead, 60 rolling seven-day outbound sends, 0 replies, 0 payments, and 0 send failures. The latest live operating observations are also descriptive: 43 seven-day page views, 6 sample clicks, 0 note-intake clicks, 0 current checkout clicks, 0 leads, 50 rolling seven-day outbound sends, 0 replies, 0 payments, and 0 send failures. The active outbound state is \texttt{hard\_paid\_test\_direct} at 10/10 progress with the reply-observation window still pending, and the product surface has been changed to reduce the remaining payment-friction path. The dashboard readiness state is an implementation-readiness measure, not a revenue result; the only revenue endpoint remains a non-owner Stripe payment. Future statistical claims require completed samples, matured observation windows, and a predeclared analysis plan.
