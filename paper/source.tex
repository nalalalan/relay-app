...
\vspace{0.35em}
\noindent\textbf{Autonomous agents are increasingly able to use tools, write messages, and execute workflows, but deployed claims of autonomy are difficult to evaluate when actions, constraints, market signals, and commercial outcomes are scattered across product analytics and human memory. We report Relay, a continuously updated deployment record for a notes-first revenue agent governed by an explicit proof contract. The system converts a broad commercial goal into a state machine with recorded bottlenecks, permitted actions, forbidden actions, proof deadlines, buyer-intent signals, payment outcomes, and compact research-journal entries. At the initial evidence lock, Relay exposed nine production routes, had all eleven checked environment requirements present, retained 30-day telemetry including 56 page views, 8 link clicks, 3 checkout clicks, and 3 lead-capture events, and recorded four compact research-journal entries. In the latest live check, read from production at approximately 2026-05-15T20:15Z, the seven-day public-intent window contained 49 page views and 6 sample clicks, while the active outbound proof remained in \texttt{stalled\_opportunity\_direct} with 9/10 active-sample sends, 0 replies, 0 payments, and USD~0.00 gross revenue. The current result is therefore an execution finding, not a demand finding: the remaining sample slot is blocked by active-experiment refill rather than buyer rejection. The dashboard now exposes only the essential operational state while the real paid-buyer gate remains missing. The current source resolves the earlier execution-accounting fault by reserving cap whenever an active sample is incomplete and no first-touch active candidate is queued. The contribution is therefore not a claim that the system already makes money. It is a falsifiable protocol for making autonomous commercial experimentation auditable enough that future success, failure, and execution regressions can be interpreted.}
...
The latest live check was read from the production Relay API at approximately 2026-05-15T20:15Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The previous \texttt{hard\_paid\_test\_direct} reply-observation window matured with zero replies and zero payments, and the controller rotated without operator choice into \texttt{stalled\_opportunity\_direct}. The new send window produced a short execution result rather than a market result: the active sample reached 9/10, the current cap is zero, and no fresh first-touch active lead is queued for the final sample slot. The current source revision treats that as an active-experiment refill bottleneck and preserves remaining cap whenever an active sample is incomplete and no active first-touch candidate is available.
...
Seven-day intent & 49 page views, 94 page-engagement events, 6 sample clicks, 0 leads. \\
