...
The latest live check was read from the production Relay API at approximately 2026-05-07T17:20Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no revenue. The prior active experiment, \texttt{stalled\_opportunity\_direct}, completed 10 of 10 sends with 0 replies and 0 payments. After its reply-observation window matured, the controller rotated to \texttt{revenue\_leak\_direct}. That is evidence of autonomous adaptation, not evidence of revenue. The new proof lane remains execution-limited: it is 1/10 complete, the daily cap is already exhausted, and the controller state is \texttt{active\_experiment\_sample}.
...
\midrule
One-day intent & 29 page views, 48 page-engagement events, 3 sample clicks, 0 real leads. \\
...
Active experiment & \texttt{revenue\_leak\_direct}; 1/10 active-sample sends observed; 10 active-proof leads queued. \\
Controller state & The bottleneck is \texttt{active\_experiment\_sample}. The next money move is to build the active proof sample before old follow-ups. \\
...
\subsection{A cap-reservation regression became part of the evidence}

The May 7 rotation exposed an execution fault rather than a market conclusion. The controller selected a new active proof lane, but only one active first-touch lead was sent before the 10/day cap was exhausted. The journaled state therefore read as \texttt{revenue\_leak\_direct}, 1/10 active progress, 0 replies, 0 payments, 10 active-proof leads still due, and 0 remaining cap. The correct interpretation is not that the new lane failed; it had not been tested.

The send-priority code was changed in commit \texttt{\seqsplit{cf74a30341c86b4678613ebfa64bb1637904f5e5}} so older direct-due follow-ups cannot consume remaining cap unless the active proof sample can be completed first. The autonomous runner prompt was also updated to treat any recurrence---daily cap exhausted while \texttt{active\_experiment\_needs\_sample} remains true---as an execution blocker. This update is recorded because an autonomous money-making system must learn from readiness and execution failures, not only from buyer outcomes.
