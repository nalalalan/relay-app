...
The buyer-facing surface is intentionally plain. The current \texttt{relaybrief.com} page presents RelayBrief as a standalone call-note cleanup service rather than as an AO Labs project route. The first screen now answers the trust problem before asking for payment: the operator is named, the price is USD~1, Stripe is identified as the checkout path, and the page states that no download, login, or password is required. The public page exposes one primary action, \emph{Open Stripe checkout}, plus a direct email link, example output, terms, and privacy pages. Notes are no longer collected on the public landing page; paid intake is separated into \texttt{relaybrief.com/client-intake.html}. These interface elements are not only conversion elements; they are also sensors. Opening the example output, clicking checkout, emailing first, and entering paid intake all create evidence used by the proof contract.
