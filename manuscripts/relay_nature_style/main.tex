...
The buyer-facing surface is intentionally direct. The current \texttt{relaybrief.com} page presents Relay as a standalone paid product rather than as an AO Labs project route: the header contains only the Relay identity and the USD~1 checkout action. A visitor can start a paid one-call test, can use a lower-priority notes form if the notes are already available, and can view a sample packet. These interface elements are not only conversion elements; they are also sensors. Viewing the sample, focusing the note field, submitting notes, clicking checkout, and entering an email all create evidence used by the proof contract.

Figure~\ref{fig:product_surface} shows the current production product surface used by the experiment. The desktop page presents the offer, USD~1 checkout, output promise, sample packet preview, and note-intake form in one view. The mobile view compresses the same path into a single-column flow. The preview is included here because future claims about buyer behaviour depend on what buyers actually saw.
