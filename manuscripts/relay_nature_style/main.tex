...
\vspace{0.35em}
\noindent\textbf{Autonomous agents are increasingly able to use tools, write messages, and execute workflows, but deployed claims of autonomy are difficult to evaluate when actions, constraints, market signals, and commercial outcomes are scattered across product analytics and human memory. We report Relay, a continuously updated deployment record for a notes-first revenue agent governed by an explicit proof contract. The system converts a broad commercial goal into a state machine with recorded bottlenecks, permitted actions, forbidden actions, proof deadlines, buyer-intent signals, payment outcomes, and compact research-journal entries. At the initial evidence lock, Relay exposed nine production routes, had all eleven checked environment requirements present, retained 30-day telemetry including 56 page views, 8 link clicks, 3 checkout clicks, and 3 lead-capture events, and recorded four compact research-journal entries. In the latest live check, read from production at approximately 2026-05-15T13:42Z, the seven-day public-intent window contained 47 page views and 6 sample clicks, while the active outbound proof had rotated into \texttt{stalled\_opportunity\_direct} with 9/10 active-sample sends, 0 replies, 0 payments, and USD~0.00 gross revenue. The current result is therefore an execution finding, not a demand finding: the send cap reached zero while the active sample remained incomplete because no fresh first-touch active lead was queued for the final slot. The dashboard exposes first-money implementation readiness while the real paid-buyer gate remains missing. The current source resolves the latest execution-accounting fault by reserving cap whenever an active sample is incomplete and no first-touch active candidate is queued. The contribution is therefore not a claim that the system already makes money. It is a falsifiable protocol for making autonomous commercial experimentation auditable enough that future success, failure, and execution regressions can be interpreted.}
...
The latest live check was read from the production Relay API at approximately 2026-05-15T13:42Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The previous \texttt{hard\_paid\_test\_direct} reply-observation window matured with zero replies and zero payments, and the controller rotated without operator choice into \texttt{stalled\_opportunity\_direct}. The new send window produced a short execution result rather than a market result: the active sample reached 9/10, the daily cap reached zero after a retry tick, and no active first-touch lead was queued for the final slot. The current source revision treats that as a cap-reservation fault and preserves remaining cap whenever an active sample is incomplete and no active first-touch candidate is available.
...
\midrule
One-day intent & 11 page views, 28 page-engagement events, 1 sample click, 0 leads. \\
Seven-day intent & 47 page views, 84 page-engagement events, 6 sample clicks, 0 leads. \\
Revenue & USD~0.00 gross revenue; 0 payments. \\
Outbound execution & The 2026-05-15 send window used the daily cap across a 9-send tick and a 1-send retry tick, but active-sample progress remained 9/10. \\
Active experiment & \texttt{stalled\_opportunity\_direct}; 9/10 active-sample sends observed; 1 active send remaining; 0 queued active first-touch leads after the retry. \\
...
The send-day accounting was revised so daily send counts begin at midnight in the configured send timezone rather than midnight UTC. The active paid-test path was also revised to expose a direct USD~1 first-money checkout, make the checkout-first route the first-screen action, add a source-visible primary-money-path marker, and preserve a lower-priority \texttt{\#send-notes} fallback without presenting it as a send-first path. Later revisions made the reply-observation state authoritative across all money-control summaries and replaced the bottom dashboard checklist with a compact first-money sequence: pay path, traffic, inspection, sample, next angle, first payment. The latest source revision adds the cap-reservation guard observed on 2026-05-15: if an active proof sample is incomplete and no active first-touch candidate is queued, normal direct follow-ups do not consume the remaining cap. This update is recorded because an autonomous money-making system must learn from readiness, buyer-friction, state-consistency, and execution-accounting failures, not only from buyer outcomes.
...
No hypothesis test was performed. The initial evidence-lock observations were descriptive: 56 page views, 18 filtered real page views, 8 link clicks, 3 checkout clicks, 3 lead-capture events, 1 filtered real lead, 60 rolling seven-day outbound sends, 0 replies, 0 payments, and 0 send failures. The latest live operating observations are also descriptive: 47 seven-day page views, 6 sample clicks, 0 leads, 0 replies, 0 payments, and USD~0.00 gross revenue. The active outbound state is \texttt{stalled\_opportunity\_direct} at 9/10 progress after the daily cap reached zero, so the current observation is a short active-sample execution fault rather than evidence of demand failure. The dashboard readiness state is an implementation-readiness measure, not a revenue result; the only revenue endpoint remains a non-owner Stripe payment. Future statistical claims require completed samples, matured observation windows, and a predeclared analysis plan.
