...
\vspace{0.35em}
\noindent\textbf{Autonomous agents are increasingly able to use tools, write messages, and execute workflows, but deployed claims of autonomy are difficult to evaluate when actions, constraints, market signals, and commercial outcomes are scattered across product analytics and human memory. We report Relay, a continuously updated deployment record for a notes-first revenue agent governed by an explicit proof contract. The system converts a broad commercial goal into a state machine with recorded bottlenecks, permitted actions, forbidden actions, proof deadlines, buyer-intent signals, payment outcomes, and compact research-journal entries. At the initial evidence lock, Relay exposed nine production routes, had all eleven checked environment requirements present, retained 30-day telemetry including 56 page views, 8 link clicks, 3 checkout clicks, and 3 lead-capture events, and recorded four compact research-journal entries. In the latest live check, read from production at approximately 2026-05-14T18:40Z, the seven-day public-intent window contained 42 page views and 6 sample clicks, while the current active outbound proof \texttt{hard\_paid\_test\_direct} had completed 10/10 active-sample sends with 0 replies, 0 payments, USD~0.00 gross revenue, and a reply-observation deadline at 2026-05-15T13:30:12Z. The dashboard now exposes first-money readiness as 8/10 gates complete: the USD~1 checkout, product route, pay-first page, notes fallback, telemetry, active sample, sample inspection, and paid-fulfillment check are complete, while buyer reply/current checkout and real paid buyer gates remain missing. Source inspection also identified a send-day accounting fault: the post-window controller could mark the local send window as missed after UTC date rollover even when active sends had occurred earlier in Eastern time. The contribution is therefore not a claim that the system already makes money. It is a falsifiable protocol for making autonomous commercial experimentation auditable enough that future success, failure, and execution regressions can be interpreted.}
...
The latest live check was read from the production Relay API at approximately 2026-05-14T18:40Z. It updates the paper beyond the initial May 4 evidence lock without changing the interpretation standard. The system has more observed intent than the initial snapshot, but it still has no real buyer revenue. The current active experiment is \texttt{hard\_paid\_test\_direct}: ten active first-touch sends were observed, the active sample was complete, and no buyer replies or payments had arrived. The most important current observation is therefore operational rather than commercial. The send sample is complete, the paid outcome is still zero, and the live controller is waiting until 2026-05-15T13:30:12Z before changing copy or targeting.
...
Controller state & The live controller reports \texttt{waiting\_for\_reply\_observation}; it blocks copy or targeting changes until 2026-05-15T13:30:12Z. \\
First-money gates & 8/10 gates complete: USD~1 checkout, product route, pay-first page, notes fallback, telemetry, 10/10 sample, 13 sample opens, and paid-fulfillment check are present; buyer reply/current checkout and real paid buyer remain missing. \\
...
The send-day accounting was revised so daily send counts begin at midnight in the configured send timezone rather than midnight UTC. The active paid-test path was also revised to expose a direct USD~1 first-money checkout, make the pay-first route the first-screen action, and preserve a working \texttt{\#send-notes} fallback for prospects who want to send rough notes before paying. This update is recorded because an autonomous money-making system must learn from readiness, buyer-friction, and execution-accounting failures, not only from buyer outcomes.
